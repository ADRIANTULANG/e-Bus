...
//<List<MonetazationPercentageModel>>
  static Future<List<MonetazationPercentageModel>>
      getMonetizationAmountDevPercentage() async {
    try {
      var response = await client.post(
        Uri.parse(
            '${AppEndpoint.endPointDomain}/get-monetization-percentage.php'),
        body: {},
      );

      if (jsonDecode(response.body)['message'] == "Success") {
        return monetazationPercentageModelFromJson(
            jsonEncode(jsonDecode(response.body)['data']));
      } else {
        return [];
      }
      ...
      Get.snackbar(
        "Get Monetization Error: Timeout",
        "Oops, something went wrong. Please try again later.",
        colorText: Colors.white,
        backgroundColor: AppColor.mainColors,
        snackPosition: SnackPosition.BOTTOM,
...
//
  static Future<List<FareAmountValueModel>>
      getMonetizationAmountFareValue() async {
    try {
      var response = await client.post(
        Uri.parse('${AppEndpoint.endPointDomain}/get-monetization-fare.php'),
        body: {},
      );

      if (jsonDecode(response.body)['message'] == "Success") {
        return fareAmountValueModelFromJson(
            jsonEncode(jsonDecode(response.body)['data']));
      } else {
        return [];
      }
  ...
  static Future updatePercentageAndFare({
    ...
    try {
      var response = await client.post(
        Uri.parse(
            '${AppEndpoint.endPointDomain}/update-monetization-percentage-and-fare.php'),
        body: {
      ...
      Get.snackbar(
        "Update Percentage Error: Timeout",
        "Oops, something went wrong. Please try again later.",
        colorText: Colors.white,
        backgroundColor: AppColor.mainColors,
        snackPosition: SnackPosition.BOTTOM,
      ...
      print(_);
      Get.snackbar(
        "Update Percentage Error: Socket",
        "Oops, something went wrong. Please try again later.",
        colorText: Colors.white,
        backgroundColor: AppColor.mainColors,
        snackPosition: SnackPosition.BOTTOM,
    ...
    try {
      var response = await client.post(
        ...
        body: {
          ...
          "adminID":
              Get.find<StorageServices>().storage.read('adminId').toString()
        },
      );
